...
describe("PrepareAttachmentUploadUseCaseのテスト", () => {
  ...
  describe("execute", () => {
    ...
    test("ファイル名が空の場合はValidationErrorを返すこと", async () => {
      const prepareAttachmentUploadUseCase =
        new PrepareAttachmentUploadUseCaseImpl({
          todoRepository: new TodoRepositoryDummy(),
          storageClient: new StorageClientDummy(),
      ...
      if (!result.success) {
        ...
        expect(result.error.message).toBe("ファイル名を入力してください");
      }
    });

    test("ファイル名が空白文字のみの場合はValidationErrorを返すこと", async () => {
      const prepareAttachmentUploadUseCase =
        new PrepareAttachmentUploadUseCaseImpl({
          todoRepository: new TodoRepositoryDummy(),
          storageClient: new StorageClientDummy(),
    ...
    test("ファイルサイズが0以下の場合はValidationErrorを返すこと", async () => {
      const prepareAttachmentUploadUseCase =
        new PrepareAttachmentUploadUseCaseImpl({
          todoRepository: new TodoRepositoryDummy(),
          storageClient: new StorageClientDummy(),
      ...
      if (!result.success) {
        expect(result.error.name).toBe("ValidationError");
        expect(result.error.message).toBe("ファイルサイズが不正です");
      }
    });

    test("ファイルサイズが負の場合はValidationErrorを返すこと", async () => {
      const prepareAttachmentUploadUseCase =
        new PrepareAttachmentUploadUseCaseImpl({
          todoRepository: new TodoRepositoryDummy(),
          storageClient: new StorageClientDummy(),
