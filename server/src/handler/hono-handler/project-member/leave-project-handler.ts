...
import type { LeaveProjectUseCase } from "@/application/use-case/project-member";
...
export const buildLeaveProjectHandler =
  ({ container }: { container: Container }) =>
  async (c: AppContext) => {
    const logger = container.get<Logger>(serviceId.LOGGER);
    const leaveProjectUseCase = container.get<LeaveProjectUseCase>(
      serviceId.LEAVE_PROJECT_USE_CASE,
    );
    ...
    try {
      ...
      const result = await leaveProjectUseCase.execute({
        projectId,
