...
- **修正には `violation-fixer` サブエージェントを使用してください**
  - 以下の情報を渡してください:
    - 違反内容（このレビュー結果の詳細セクション）
    - ポリシーディレクトリ: {このレビューで使用したポリシーディレクトリパス}
  - **他のguardrails-reviewerの指摘は別のviolation-fixerで対応してください（並列処理のため）**
- 原則に基づく指摘（⚠️）がある場合は、その観点をポリシーとして明文化することを検討してください
- 申し立て・新設提案は、理由と提案内容を含めて、ポリシーファイルの修正提案として提出してください
