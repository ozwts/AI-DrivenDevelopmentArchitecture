...
以下の品質ゲートを順番に実行してください:

### Step 1: 静的解析（最速フィードバック）
...
guardrails-reviewerサブエージェントで再レビュー:
- ポリシーディレクトリ: {元のポリシーディレクトリ}
- 対象ファイル: {修正したファイル}

→ 違反があればviolation-fixerで修正

### 循環

**違反ゼロになるまで Step 1 → Step 2 を繰り返してください。**
```
