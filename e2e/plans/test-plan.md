...
#### 2.6. 正常系: 添付ファイルをダウンロードできる

**File:** `e2e/tests/todos/download-attachment.spec.ts`

**Steps:**
  1. 添付ファイルがあるTODOの詳細ページ（/todos/{todoId}）に遷移する
  2. 添付ファイルセクションでファイル名をクリック

**Expected Results:**
  - ファイルのダウンロードが開始される
  - ダウンロードされたファイルが正しい内容を持つ
  - ダウンロード中にエラーが発生しない

#### 2.7. 正常系: 既存TODOに添付ファイルを追加できる

**File:** `e2e/tests/todos/add-attachment-to-existing-todo.spec.ts`

**Steps:**
  1. 添付ファイルがないTODOの詳細ページ（/todos/{todoId}）に遷移する
  2. 「ファイルを追加」ボタンをクリック
  3. テスト用ファイルを選択
  4. アップロード完了を待つ

**Expected Results:**
  - 成功トーストメッセージ「ファイルをアップロードしました」が表示される
  - 添付ファイルセクションにアップロードしたファイルが表示される
  - ファイル名とサイズが正しく表示される

#### 2.8. 正常系: 添付ファイルを削除できる

**File:** `e2e/tests/todos/delete-attachment.spec.ts`
...
**Expected Results:**
  - 成功トーストメッセージ「ファイルを削除しました」が表示される
  - 削除したファイルが添付ファイルセクションから消える
  - TODO自体は削除されない

#### 2.9. エッジケース: 期限をクリアできる（PATCH 3値セマンティクス）
