...
- **機能的凝集**: モジュール内部は「目的が同じ」要素をコロケーションし、変更の影響範囲を構造的に限定（詳細は `module-cohesion-principles.md`）
- **構造的単純性**: Simple（絡んでいない）を優先し、Easy（慣れている）に惑わされない（詳細は `simplicity-principles.md`）
...
**モジュール凝集**: モジュール内部のコード配置は `module-cohesion-principles.md` に従う
...
インターフェース先行定義、レイヤー別分担でコンフリクトを最小化

### モジュール凝集

モジュール内部のコード配置・共通化の判断は `module-cohesion-principles.md` に従う
