...
export const WORKFLOW_RESPONSIBILITIES: WorkflowResponsibility[] = [
  {
    ...
    toolDescription:
      "Workflow task management. action: 'plan' (invoke subagent for planning), 'set' (register/overwrite tasks), 'done' (mark task complete), 'list' (show all tasks), 'clear' (clear all tasks).",
    inputSchema: {
      action: z
    ...
    handler: async (input, guardrailsRoot): Promise<string> => {
      ...
      switch (action) {
        ...
        case "set": {
          ...
          return formatSetResult(tasks.length);
        }
        ...
        case "list": {
          ...
          return formatTaskList(tasks, runbooksDir);
        }
