...
| 成果物 | 実装先 | 粒度 |
|--------|--------|------|
| ポリシー修正判断 | - | 1タスク（修正不要の確認も含む） |
| meta.json | `guardrails/policy/{category}/{domain}/meta.json` | ポリシーごとに含む |
| 概要ドキュメント | `guardrails/policy/{category}/{domain}/10-*-overview.md` | ポリシーごとに1タスク |
| 詳細ルール | `guardrails/policy/{category}/{domain}/20-*.md` 以降 | ポリシーごとに含む |
| 人間によるレビュー | - | 1タスク（必須） |

### タスク分割ルール

1. **修正不要の場合**: 「ポリシー修正不要を確認」の1タスクのみ
2. **修正/新設の場合**: ポリシーごとに1タスク + 人間レビュー
3. **必須の終了タスク**: 「計画見直し」「コミット・PR更新」を最後に追加

### タスク例（新規ポリシー「notification」追加の場合）

```
1. 既存ポリシーを確認し、修正・新設の要否を判断
2. guardrails/policy/server/notification/ にポリシーを起草
3. 人間によるポリシーレビューを依頼
4. 後続フェーズの作業計画を見直す
5. Policyフェーズの成果物をコミット・プッシュ・PR更新
```

### タスク例（修正不要の場合）

```
1. 既存ポリシーで対応可能であることを確認
2. 後続フェーズの作業計画を見直す
3. Policyフェーズの成果物をコミット・プッシュ・PR更新
```
...
**レビュー観点:** `guardrails/constitution/` との整合性を確認
...
```bash
git add -A
git commit -m "policy({scope}): {message}"
