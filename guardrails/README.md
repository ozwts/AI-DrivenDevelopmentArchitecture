# Guardrails - 三権分立システム

AIが自走するガードレールに三権分立の概念を導入したシステムです。

## 概要

```
Constitution（憲法: 最高原則）
    ↓
Policy（立法: ルール定義）
    ↓ [MCPサーバーが並列読み込み]
Review（司法: Claude Agent SDKで審査実行）
    ↓ [レビュー結果を返す]
```

## ディレクトリ構造

```
guardrails/
├── package.json          # ビルド設定
├── tsconfig.json         # TypeScript設定
├── index.ts              # MCPサーバー（統合エントリーポイント）
├── constitution/         # 憲法（最高原則・価値・法令・倫理）
│   ├── validation-principles.md     # バリデーション原則（MECE）
│   └── observability-principles.md  # 可観測性原則（AI自律開発と人間の責任担保）
├── policy/              # 立法（憲法を具体化したルール群）
│   ├── web/
│   │   └── test-strategy/
│   │       ├── 00-test-coverage-requirements.md
│   │       ├── 10-test-strategy-overview.md
│   │       ├── 20-component-test.md
│   │       └── 30-snapshot-test.md
│   └── server/
│       └── domain-model/
│           ├── 10-domain-model-overview.md
│           ├── 20-entity-design.md
│           ├── 30-repository-interface.md
│           └── 40-aggregate-pattern.md
├── review/              # 司法（Policyに従って審査するシステム）
│   ├── web/
│   │   └── test-strategy/
│   │       ├── index.ts              # MCPハンドラー
│   │       ├── test-reviewer.ts      # レビュー実行
│   │       ├── coverage-checker.ts   # カバレッジチェック
│   │       └── formatter.ts          # フォーマッター
│   └── server/
│       └── domain-model/
│           ├── index.ts              # MCPハンドラー
│           ├── domain-reviewer.ts    # レビュー実行
│           └── formatter.ts          # フォーマッター
└── procedure/           # 行政（Policyに従って動く実行・オペレーション）
```

## 三権分立の対応

| 役割 | フォルダ      | 説明                                   | 担当    |
| ---- | ------------- | -------------------------------------- | ------- |
| 憲法 | constitution/ | 最高原則・価値・法令・倫理             | 人間    |
| 立法 | policy/       | 憲法を具体化したルール群               | 人間    |
| 司法 | review/       | Policyに従って審査するシステム（MCP）  | AI      |
| 行政 | procedure/    | Policyに従って動く実行・オペレーション | AI/人間 |

## セットアップ

### 1. 依存関係のインストール

```bash
cd guardrails
npm install
```

### 2. 環境変数の設定

```bash
export ANTHROPIC_API_KEY="your-api-key-here"
```

### 3. Claude Desktop設定

`~/Library/Application Support/Claude/claude_desktop_config.json` に以下を追加：

```json
{
  "mcpServers": {
    "guardrails": {
      "command": "npx",
      "args": ["tsx", "/Users/your-username/path/to/guardrails/index.ts"],
      "env": {
        "ANTHROPIC_API_KEY": "your-api-key-here"
      }
    }
  }
}
```

## 使い方

### MCPツール1: `review_web_tests`

フロントエンドのテストファイルがテスト戦略ポリシーに準拠しているかを審査します。

#### 対象ファイル

- コンポーネントテスト（`*.ct.test.tsx`）
- スナップショットテスト（`*.ss.test.ts`）

#### 入力

```json
{
  "targetFilePaths": [
    "/path/to/TodoForm.ct.test.tsx",
    "/path/to/TodoCard.ct.test.tsx",
    "/path/to/ProjectForm.ct.test.tsx"
  ]
}
```

#### 処理フロー

1. **全ファイルのポリシーを収集**（重複排除）
   - ファイル種別判定（.ct.test.tsx / .ss.test.ts）
   - 必要なポリシーを並列読み込み
2. **1回のClaude API呼び出しで全ファイルをレビュー**
   - read_fileツールで各ファイルを読み込み
   - 全ファイルをまとめて審査
3. **結果を構造化**して返す

#### 出力例

```markdown
# 📝 個別テストファイルレビュー

## サマリー

- 総ファイル数: 3
- 成功: 3
- 失敗: 0

---

## TodoForm.ct.test.tsx

### 適用されたポリシー

- 10-test-strategy-overview.md
- 20-component-test.md

### レビュー

[Claudeによるレビュー内容]
...
```

### MCPツール2: `review_domain_models`

サーバー側のドメインモデルファイルがドメインモデルポリシーに準拠しているかを審査します。

#### 対象ファイル

- エンティティ（`*.ts`、ただし`*-repository.ts`以外）
- リポジトリインターフェース（`*-repository.ts`）
- 除外: `*.small.test.ts`、`*.dummy.ts`

#### 入力

```json
{
  "targetFilePaths": [
    "/path/to/server/src/domain/model/todo/todo.ts",
    "/path/to/server/src/domain/model/todo/todo-repository.ts"
  ]
}
```

#### 処理フロー

1. **全ファイルのポリシーを収集**（重複排除）
   - ファイル種別判定（エンティティ / リポジトリ）
   - 必要なポリシーを並列読み込み
2. **1回のClaude API呼び出しで全ファイルをレビュー**
   - read_fileツールで各ファイルを読み込み
   - 全ファイルをまとめて審査
3. **結果を構造化**して返す

#### 出力例

```markdown
# 📝 ドメインモデルレビュー

## サマリー

- 総ファイル数: 2
- 成功: 2
- 失敗: 0

---

## todo.ts

### 適用されたポリシー

- 10-domain-model-overview.md
- 20-entity-design.md
- 40-aggregate-pattern.md

### レビュー

[Claudeによるレビュー内容]
...
```

## ファイル種別とポリシーの対応

### Web テスト戦略（`review_web_tests`）

| ファイル種別    | 選択されるポリシー（並列読み込み）                                        |
| --------------- | ------------------------------------------------------------------------- |
| `*.ct.test.tsx` | `10-test-strategy-overview.md` + `20-component-test.md`                   |
| `*.ss.test.ts`  | `10-test-strategy-overview.md` + `30-snapshot-test.md`                    |
| カバレッジ      | `00-test-coverage-requirements.md`（ページディレクトリごとに1回チェック） |

### サーバー ドメインモデル（`review_domain_models`）

| ファイル種別                    | 選択されるポリシー（並列読み込み）                                                       |
| ------------------------------- | ---------------------------------------------------------------------------------------- |
| エンティティ（`*.ts`）          | `10-domain-model-overview.md` + `20-entity-design.md` + `40-aggregate-pattern.md`        |
| リポジトリ（`*-repository.ts`） | `10-domain-model-overview.md` + `30-repository-interface.md` + `40-aggregate-pattern.md` |

**重要**:

- 各ツールは対応するpolicyフォルダ内のファイルを並列で読み込みます
- 各ファイルがファイル種別に応じて個別にポリシーを選択されます

## 使用例

### Claude Code での使用

Claude CodeからMCPツールを呼び出して、作成・修正したファイルをレビューします。

```typescript
// フロントエンドテストのレビュー
await mcp__guardrails__review_web_tests({
  targetFilePaths: [
    "/path/to/TodoForm.ct.test.tsx",
    "/path/to/TodoCard.ct.test.tsx",
  ],
});

// ドメインモデルのレビュー
await mcp__guardrails__review_domain_models({
  targetFilePaths: [
    "/path/to/server/src/domain/model/todo/todo.ts",
    "/path/to/server/src/domain/model/todo/todo-repository.ts",
  ],
});
```

## 拡張方法

### 新しいポリシーの追加

1. `policy/{layer}/{category}/` に新しいポリシーファイルを追加（例: `50-xxx.md`）
2. `review/{layer}/{category}/` のレビューロジックを更新

### 新しいファイルタイプのサポート

`review/{layer}/{category}/*-reviewer.ts` の `selectPolicies()` 関数に条件を追加：

```typescript
if (fileName.endsWith(".new-type.ts")) {
  policyFiles = [
    path.join(policyBase, "10-overview.md"),
    path.join(policyBase, "50-new-policy.md"),
  ];
}
```

### 新しいレイヤー/カテゴリの追加

```
policy/
├── web/
│   └── test-strategy/
├── server/
│   ├── domain-model/        # 実装済み
│   └── use-case/            # ← 新カテゴリ
└── infra/
    └── terraform/           # ← 新レイヤー

review/
├── web/
│   └── test-strategy/
├── server/
│   ├── domain-model/        # 実装済み
│   └── use-case/            # ← 対応するレビュー実装
│       └── index.ts
└── infra/
    └── terraform/
        └── index.ts
```

実装後、`guardrails/index.ts` にMCPツールを登録します。

## 開発

```bash
# 開発モード（ビルド + 実行）
npm run dev

# 型チェック
npm run validate:tsc

# 実行のみ
npm start
```

## アーキテクチャのポイント

1. **1ショットレビュー**
   - 全ファイルを1回のClaude API呼び出しでレビュー
   - コスト効率とレスポンス時間の最適化
   - ポリシーファイルの読み込みはPromise.allで並列化

2. **ポリシー選択の自動化**
   - ファイル拡張子からポリシーを自動選択
   - 重複ポリシーを排除して効率的に読み込み

3. **Claude Agent SDK統合**
   - MCPツール内でClaude APIを直接呼び出し
   - read_fileツールで対象ファイルを動的に読み込み
   - レビュー結果を構造化して返却
