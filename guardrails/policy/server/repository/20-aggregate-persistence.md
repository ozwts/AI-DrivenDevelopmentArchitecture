...
```
[ ] 子エンティティ専用リポジトリを作っていない
[ ] save()で集約ルートと子エンティティを一括保存
[ ] findById()で集約ルートと子エンティティを一括取得
[ ] トランザクションで親と子の整合性を保証
[ ] データ変換関数でドメインモデル ⇔ DB形式を変換
[ ] 親IDを子エンティティのドメインモデルに含めていない（DB形式のみ）
[ ] UnitOfWorkをサポート（オプション）
[ ] アグリゲート境界の判断基準を確認
[ ] トランザクション操作数が100以内であることを確認
```
