...
| ファイル | 検証内容 |
|---------|---------|
| [`no-console-log.ts`](../../horizontal/server/logger/no-console-log.ts) | console.logの直接使用禁止 |
| [`no-domain-logging.ts`](../../horizontal/server/logger/no-domain-logging.ts) | ドメイン層でのログ出力禁止 |
...
| ファイル | 検証内容 |
|---------|---------|
| [`logger-responsibility.md`](../../horizontal/server/logger/logger-responsibility.md) | Logger使用の責務（機密情報、構造化ログ、ログレベル） |
