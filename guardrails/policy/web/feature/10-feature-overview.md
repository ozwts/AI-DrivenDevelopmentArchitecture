...
Featureは**アプリケーション固有の概念を持ち、3つ以上のrouteで共通利用される横断的機能**を配置する場所である。
...
|---------|--------|
| 3+route | `features/` |
| 1-2route | 各routeに個別配置（重複許容） |
| 親子route間共有 | `{feature}/_shared/`（詳細は`route/30-shared-placement.md`）|
...
**注意**:
- `components/` の設計詳細は `../component/10-component-overview.md` を参照
- `hooks/` の設計詳細は `../hooks/10-hooks-overview.md` を参照
...
- `../component/10-component-overview.md`: コンポーネント設計概要
