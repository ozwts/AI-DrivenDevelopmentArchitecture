...
export type StaticAnalysisResponsibility = {
  ...
  inputSchema: {
    ...
    analysisType: z.ZodDefault<z.ZodOptional<z.ZodEnum<["type-check", "lint", "both"]>>>;
  };
};
...
export const REVIEW_RESPONSIBILITIES: ReviewResponsibility[] = [
  {
    ...
    scope: {
      ...
      aspects:
        "DDD原則、不変条件、バリデーション、集約設計、リポジトリインターフェース設計",
    },
    toolDescription:
  ...
  },
  {
    ...
    scope: {
      ...
      aspects:
        "テスト戦略適合性、コンポーネントテストのアサーション、スナップショットテストの適用範囲",
    },
    toolDescription:
  ...
  },
  {
    ...
    scope: {
      ...
      aspects:
        "Result型パターン、ビジネスルール検証、executeメソッド単一責任、ドメインモデル貧血症防止、命名規則、依存性注入",
    },
    toolDescription:
  ...
  },
  {
    ...
    scope: {
      ...
      aspects:
        "HTTPリクエスト/レスポンス変換、Zodバリデーション、エラーハンドリング、薄いアダプター原則、命名規則、ファイル配置",
    },
    toolDescription:
  ...
  },
  {
    ...
    scope: {
      ...
      aspects:
        "集約単位の永続化、データ変換（ドメインモデル⇔DB）、トランザクション管理、Result型返却、DynamoDBパターン、命名規則",
    },
    toolDescription:
  ...
  },
  {
    ...
    scope: {
      ...
      aspects:
        "型レベルバリデーション定義、エンドポイント設計、HTTPステータスコード、enum定義、Single Source of Truth原則、命名規則",
    },
    toolDescription:
